//! 服务器启动模块
//!
//! 包含完整的服务器初始化和启动逻辑，可被独立二进制（main.rs）
//! 和 Tauri 应用（src-tauri/src/main.rs）共用。
//! 不包含任何业务逻辑，仅做路由装配和服务器生命周期管理。
...
async fn init_web_auth_state(config: &crate::config::WebAuthConfig) -> Arc<WebAuthState> {
    use crate::web_auth::create_auth_store;
...
///
/// 此函数包含完整的服务器初始化和启动逻辑：
/// 日志、配置、AppState、代理探测、Web 认证、路由、绑定、优雅关闭。
/// 可被独立二进制（backend/src/main.rs）和 Tauri 应用（src-tauri/src/main.rs）共用。
pub async fn start_server() -> anyhow::Result<()> {
    ...
    info!("Baidu Netdisk Rust v2.1.2 启动中...");
    ...
    // 必须在 load_initial_session 之前执行，否则预热和 BDUSS 验证会走死代理
    {
        ...
        if proxy.proxy_type != crate::common::ProxyType::None {
            ...
            match crate::common::probe_proxy(&proxy_clone).await {
                Ok(()) => {
                    info!("✓ 启动代理探测成功");
                    app_state.fallback_mgr
                        .set_runtime_status(crate::common::ProxyRuntimeStatus::Normal)
                        .await;
                }
                Err(e) => {
