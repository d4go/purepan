# 多阶段构建 Dockerfile
# 基础镜像：node:18-alpine（前端构建）、rust:1.87-slim（后端构建）、debian:bookworm-slim（运行时）
# Stage 1: 前端构建（基于 Node.js 18 Alpine）
...
# Stage 2: 后端构建（基于 Rust 1.87 Slim）
...
# Stage 3: 运行时镜像（基于 Debian Bookworm Slim）
...
# 复制后端可执行文件
COPY --from=backend-builder /app/target/release/PurePan /app/PurePan
...
# 健康检查
# 路径: http://localhost:18888/health（与 docker-compose.yml 保持一致）
