    ...
    beforeEnter: async (_to: RouteLocationNormalized, _from: RouteLocationNormalized, next: NavigationGuardNext) => {
      ...
      if (!baiduAuthChecked) {
        ...
        try {
          ...
          console.warn('[router] 根路由 fetchAccountList 失败:', err)
        }
      }
...
/** 启动 bootstrap 已完成认证检查时，避免首导航重复发起阻塞请求。 */
export function primeAuthCheckFlags() {
  webAuthInitialized = true
  baiduAuthChecked = true
}

/**
 * 重置认证检查标志（用于退出登录后重新检查）
 */
export function resetAuthCheckFlags() {
  baiduAuthChecked = false
  webAuthInitialized = false
}
