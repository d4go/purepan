  ...
  <div class="downloads-container">
    ...
    <div class="task-container">
      ...
      <div v-else class="task-list">
        <div
            ...
            @contextmenu.prevent.stop="handleRowContextMenu(item, $event)"
        >
          <!-- 主行：图标 + 文件名 + 状态 + 统计 + 操作 -->
          <div class="task-row-main">
            <div class="task-row-info">
              ...
              <span class="filename" :title="item.type === 'folder' ? item.name : getDisplayFilename(item)">
              ...
              <el-tag v-if="item.is_encrypted" type="info" size="small" class="encrypt-tag">
            ...
            <div class="task-row-stats">
              <span v-if="item.type === 'folder'" class="stat-item">{{ item.completed_files }}/{{ item.total_files }} 文件</span>
              <span class="stat-item">{{ formatFileSize(item.downloaded_size || 0) }} / {{ formatFileSize(item.total_size || 0) }}</span>
              <span v-if="item.status === 'downloading' || item.status === 'scanning'" class="stat-item speed">{{ formatSpeed(item.speed || 0) }}</span>
              <span v-if="item.status === 'downloading' && item.type === 'file'" class="stat-item">
                {{ formatETA(calculateETA({ total_size: item.total_size || 0, downloaded_size: item.downloaded_size || 0, speed: item.speed || 0 } as any)) }}
              </span>
            </div>

            <!-- 操作按钮 -->
            <div class="task-row-actions">
              <el-tooltip v-if="item.transfer_task_id" content="查看转存" placement="top">
                <el-button size="small" type="info" plain circle @click="goToTransferTask(item.transfer_task_id)">
                  <el-icon><Share/></el-icon>
                </el-button>
              </el-tooltip>
              <el-tooltip v-if="item.type === 'folder'" content="详情" placement="top">
                <el-button size="small" circle @click="showFolderDetail(item)">
                  <el-icon><List/></el-icon>
                </el-button>
              </el-tooltip>
              <el-tooltip v-if="item.status === 'downloading' || item.status === 'scanning'" content="暂停" placement="top">
                <el-button size="small" circle @click="handlePause(item)">
                  <el-icon><VideoPause/></el-icon>
                </el-button>
              </el-tooltip>
              <el-tooltip v-if="item.status === 'paused' || item.status === 'failed'" :content="item.status === 'failed' ? '重试' : '继续'" placement="top">
                <el-button size="small" type="primary" circle @click="handleResume(item)">
                  <el-icon><VideoPlay/></el-icon>
                </el-button>
              </el-tooltip>
              <el-tooltip v-if="item.status === 'completed'" content="打开文件夹" placement="top">
                <el-button size="small" type="success" circle @click="openDownloadFolder(item)">
          ...
          <!-- 路径 -->
          <div class="task-row-path" :title="item.type === 'folder' ? item.remote_root : item.remote_path">
          ...
          <!-- 进度条 -->
          <div class="task-row-progress" v-if="item.status !== 'decrypting'">
            <el-progress
    ...
    <div class="status-bar">
      <div class="status-left">
        <span class="status-item">总任务: {{ downloadItems.length }}</span>
    ...
    >
      <div class="context-menu-header" v-if="contextMenuTarget">
        <el-icon :size="14"><Document v-if="contextMenuTarget.type !== 'folder'"/><Folder v-else/></el-icon>
        <span class="truncate">{{ contextMenuTarget.type === 'folder' ? contextMenuTarget.name : getDisplayFilename(contextMenuTarget) }}</span>
      </div>
      <div class="context-menu-divider"></div>
      <div class="context-menu-item" @click="handleContextMenuAction('pause')" v-if="contextMenuTarget && (contextMenuTarget.status === 'downloading' || contextMenuTarget.status === 'scanning')">
        <el-icon><VideoPause /></el-icon>
        <span>暂停</span>
      </div>
      <div class="context-menu-item" @click="handleContextMenuAction('resume')" v-if="contextMenuTarget && (contextMenuTarget.status === 'paused' || contextMenuTarget.status === 'failed')">
        <el-icon><VideoPlay /></el-icon>
        <span>继续</span>
      </div>
      <div class="context-menu-item" @click="handleContextMenuAction('open')" v-if="contextMenuTarget && contextMenuTarget.status === 'completed'">
        <el-icon><FolderOpened /></el-icon>
        <span>打开文件夹</span>
      </div>
      <div class="context-menu-divider"></div>
...
import { getParentFolder, openFolder } from '@/utils/shellAdapter'
...
// 打开下载目录：文件任务打开所在目录，文件夹任务打开下载根目录。
function openDownloadFolder(item: DownloadItemFromBackend) {
  const path = item.type === 'folder'
      ? (item.local_root || '')
      : getParentFolder(item.local_path || '')
...
const contextMenuTarget = ref<DownloadItemFromBackend | null>(null)
...
function handleRowContextMenu(item: DownloadItemFromBackend, event: MouseEvent) {
  event.preventDefault()
  event.stopPropagation()
  contextMenuTarget.value = item
  // 估算菜单高度（5个菜单项 ≈ 200px），当靠近底部时向上弹出
...
async function handleContextMenuAction(action: string) {
  ...
  switch (action) {
    case 'pause':
      if (target.status === 'downloading' || target.status === 'scanning') {
        await handlePause(target)
      }
      break
    case 'resume':
...
function handleGlobalClick(event: MouseEvent) {
  if (contextMenuVisible.value) {
    const menuEl = document.querySelector('.downloads-container .context-menu')
...
.task-row {
  ...
  @keyframes highlight-fade {
    0% { background: var(--el-color-warning-light-8); }
    70% { background: var(--el-color-warning-light-9); }
