    ...
    >
      <el-form :model="form" label-width="100px" :rules="formRules" ref="formRef">
        ...
        <el-form-item v-if="form.poll_config.mode === 'interval'" label="间隔（秒）">
          ...
          <span style="margin-left: 8px; color: var(--app-text-secondary); font-size: 12px">最少 600 秒（10 分钟）</span>
        </el-form-item>
        ...
        <el-form-item label="删除缺失">
          ...
          <span style="margin-left: 8px; color: var(--app-text-secondary); font-size: 12px">分享者删除文件时同步删除目标副本</span>
        </el-form-item>
      </el-form>
      <template #footer>
      ...
      <div v-if="currentRun" class="run-detail">
        <el-descriptions :column="2" border size="small">
          ...
          <el-descriptions-item label="错误" v-if="currentRun.error">
            <span style="color: var(--el-color-danger)">{{ currentRun.error }}</span>
          </el-descriptions-item>
        </el-descriptions>
        ...
        >
          <div @click="openRun(r.id)" class="run-item">
            ...
            <div class="run-stats">
              ...
              <span v-if="runSkippedCount(r) > 0" style="color: var(--el-color-warning)"> 跳过 {{ runSkippedCount(r) }}</span>
              <span v-if="r.failed_count > 0" style="color: var(--el-color-danger)"> 失败 {{ r.failed_count }}</span>
            </div>
          </div>
        </el-timeline-item>
      </el-timeline>
...
.config-card {
  ...
  &.is-disabled { border-left-color: var(--app-text-tertiary); }
  &.active { box-shadow: 0 0 0 1px var(--app-accent) inset; }
...
.config-title {
  ...
  margin-bottom: 8px;
  flex-wrap: wrap;
  .direction-icon { flex-shrink: 0; color: var(--app-accent); }
  .config-name { font-size: 16px; font-weight: 500; color: var(--app-text-primary); }
}
.config-path {
  font-size: 12px;
  ...
  align-items: center;
  gap: 4px;
  .dot { color: var(--app-text-tertiary); }
...
.task-progress-header {
  ...
  .toggle-icon { margin-left: auto; color: var(--app-text-secondary); }
}
.task-status-info {
  display: flex;
  align-items: center;
  gap: 8px;
  .task-status-text { font-size: 14px; font-weight: 500; color: var(--app-text-primary); }
  .status-icon.text-blue-500 { color: var(--app-accent); }
...
.subtask-head {
  ...
  .subtask-stat { font-size: 12px; color: var(--app-text-secondary); flex-shrink: 0; }
}
...
.no-active-task {
  ...
  background: var(--app-surface-hover);
  border-radius: 8px;
  .idle-text { font-size: 13px; color: var(--app-text-secondary); }
}
...
.target-tip { margin-left: 8px; font-size: 12px; color: var(--app-text-secondary); }

.sync-mode-editor {
  width: 100%;
  .sync-mode-hint { margin: 8px 0 4px; font-size: 12px; color: var(--app-text-secondary); }
  .mode-target-row {
    display: flex;
    align-items: center;
    margin-top: 8px;
    ...
    .mode-target-value {
      ...
      &.is-placeholder { color: var(--app-text-tertiary); }
    }
  }
}

.run-item { cursor: pointer; &:hover { color: var(--app-accent); } }
.run-stats { font-size: 12px; color: var(--app-text-secondary); margin-top: 2px; }
...
.tree-picker-hint { margin-right: 12px; color: var(--app-text-secondary); font-size: 12px; }
.tree-node { display: inline-flex; align-items: center; gap: 4px; }
.tree-size { margin-left: 6px; color: var(--app-text-secondary); font-size: 11px; }
...
:deep(.el-descriptions) {
  .el-descriptions__header { margin-bottom: 12px; }
  .el-descriptions__label { font-weight: 500; }
}
</style>
