        ...
        <el-alert type="info" :closable="false" show-icon style="margin-bottom: 16px">
          ...
          <template #default>
            <div style="line-height: 1.8">
              ...
              <span style="color: var(--app-text-secondary)">提示：只有一个账号时此设置不影响速度（不用分配）；数值只看相对比例，1 : 3 : 5 与 2 : 6 : 10 效果相同。</span>
            </div>
          </template>
        </el-alert>
...
.triple-bar-fill {
  ...
  transition: width 0.3s cubic-bezier(0.25, 0.1, 0.25, 1), background 0.3s cubic-bezier(0.25, 0.1, 0.25, 1);
  ...
  &.status-below_base {
    background: var(--el-color-success); // 绿色：还在 base 之下，处于 P0 阶段
  }

  &.status-above_base {
    background: var(--el-color-warning); // 橙色：在 base 之上 vip_cap 之下，处于 P1 借调阶段
  }

  &.status-full {
    background: var(--el-color-danger); // 红色：撞 vip_cap，已满载
  }
}
