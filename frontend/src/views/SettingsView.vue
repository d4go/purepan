            ...
            <el-skeleton :loading="loading" :rows="8" animated>
              ...
              >
                ...
                <el-card id="section-transfer" class="setting-card" shadow="hover">
                  <template #header>
                    <div class="card-header">
                      <el-icon :size="20" color="var(--app-text-secondary)">
                        <Share />
                      </el-icon>
                      <span>转存配置</span>
                    </div>
                ...
                <el-card id="section-encryption" class="setting-card" shadow="hover">
                  ...
                  <el-alert
                      type="error"
                      :closable="false"
                      style="margin-bottom: 20px; border-left: 4px solid var(--el-color-danger);"
                  >
                    <template #title>
                ...
                <el-card id="section-backup" class="setting-card" shadow="hover">
                  ...
                  <div v-if="triggerConfig.upload_trigger.watch_enabled" class="nested-config">
                    <el-divider content-position="left" style="margin: 12px 0">
                      <span style="font-size: 12px; color: var(--app-text-secondary)">监听兜底设置（防止监听遗漏）</span>
                    </el-divider>
                  ...
                  <el-alert type="info" :closable="false" show-icon>
                    ...
                    <template #default>
                      <div style="font-size: 12px; line-height: 1.8; color: var(--app-text-secondary);">
                ...
                <!-- 关于信息 -->
                <el-card id="section-ui" class="setting-card" shadow="hover">
                  <template #header>
                    <div class="card-header">
                      <el-icon :size="20" color="var(--app-accent)">
                        <Monitor />
                  ...
                  <el-form-item label="关闭按钮行为">
                    <el-radio-group v-model="closeBehavior">
                      <el-radio value="close">直接关闭软件</el-radio>
                      <el-radio value="minimize_to_tray">最小化到系统托盘</el-radio>
                    </el-radio-group>
                    <div class="form-tip">
                      选择"最小化到系统托盘"后，点击窗口关闭按钮时软件将隐藏在系统托盘中继续运行
                    </div>
                  </el-form-item>
                </el-card>
...
const closeBehavior = ref<'close' | 'minimize_to_tray'>('close')

const contentRef = ref<HTMLElement>()

// ?tab 定位支持的合法 section id
const sectionIds = [
  'section-accounts', 'section-budget', 'section-server', 'section-auth',
  'section-download', 'section-upload', 'section-conflict', 'section-transfer',
  'section-encryption', 'section-backup', 'section-proxy', 'section-ui', 'section-about',
]
...
async function loadConfig() {
  ...
  try {
    ...
    // 初始化 UI 配置
    if (formData.value?.ui?.close_behavior) {
      closeBehavior.value = formData.value.ui.close_behavior
    } else {
      closeBehavior.value = 'close'
      if (formData.value) {
        if (!formData.value.ui) {
          formData.value.ui = { close_behavior: 'close' }
        }
      }
    }
...
async function handleSave() {
  ...
  try {
    ...
    formData.value.ui.close_behavior = closeBehavior.value
...
onMounted(() => {
  ...
  // 支持 ?tab=accounts 直接定位到指定 section
