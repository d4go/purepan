...
export const useAuthStore = defineStore('auth', () => {
  ...
  /**
   * 当前用户完整信息（合并多账号 activeAccount + 旧 user 兼容字段）。
   *
   * 多账号路径（fetchAccountList / WS snapshot / switchAccount）只填充
   * `accounts` + `activeUid`，不填充旧 `user` 字段；应用重启后 `user` 为 null，
   * 导致直接绑定 `user` 的旧组件（如 UserProfileDialog）显示"未知用户"。
   *
   * 此 computed 优先用 activeAccount 提供 nickname/username/avatar/vip_type/uid，
   * 再用 user 补充 login_time/total_space/used_space 等扩展字段，保证旧组件
   * 在纯多账号路径下也能正常展示。
   */
  const currentUser = computed<UserAuth | null>(() => {
    ...
    if (acc) {
      return {
        ...
        vip_type: acc.vip_type ?? undefined,
        // 扩展字段：仅旧 user 路径才有，多账号路径下为 undefined（UI 显示 '-'）
        login_time: u?.login_time,
        total_space: u?.total_space,
  ...
  // 开始轮询二维码状态
  //
  // 使用递归 setTimeout 代替 setInterval，确保前一次 poll 完成后才调度
  // 下一次。setInterval 不等 await 完成，会导致多个 poll 并发执行，
  // 在后端响应慢时形成请求堆积和竞态（例如 success 被多次触发）。
  function startPolling(
    ...
    const poll = async () => {
      ...
      try {
        const status = await getQRCodeStatus(qrcode.value.sign, addMode, isFirstPoll)
        isFirstPoll = false

        // 请求返回后再次检查轮询状态（可能在 await 期间被停止）
        ...
        switch (status.status) {
          case 'success':
            // 登录成功
            stopPolling()
            // 后端 success 响应已包含完整 user 信息，无需再调 fetchUserInfo
            if (status.user) {
      ...
      // 调度下一次轮询（递归 setTimeout，确保前一次完成后才发起下一次）
      if (isPolling.value) {
        pollingTimer.value = window.setTimeout(poll, 1500)
      }
    }
