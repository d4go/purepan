...
function handleOpen() {
  ...
  if (props.mode === 'download') {
    // 下载模式：优先使用 initialPath，其次 defaultDownloadDir，默认 C:\
    const initialPath = props.initialPath || props.defaultDownloadDir || 'C:\\'
    store.jumpToPath(initialPath).then((success) => {
      if (!success) {
        // C:\ 不存在或跳转失败，回退到根目录列表
        store.loadDirectory('')
      }
